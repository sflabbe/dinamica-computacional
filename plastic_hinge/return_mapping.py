...
def project_onto_polytope_2d(
    ...
    # Candidates: project onto each constraint (single active), and intersections (two actives)
    ...
    for i in range(A.shape[0]):
        ...
        try:
            ...
            continue
        if np.all(A @ x - b <= tol + 1e-12):
            candidates.append(ProjectionResult(x=x, lam=lam, active=np.array([i], int), obj=obj(x)))

    # Two-active intersections: try all pairs (O(m^2) cheap in 2D)
    ncon = int(A.shape[0])
    for i in range(ncon):
    ...
    if not candidates:
        ...
        for _ in range(25):
            ...
            if not improved and np.all(A @ x - b <= tol + 1e-12):
                break
        act = np.where(np.abs(A @ x - b) <= max(tol, 1e-8))[0]
