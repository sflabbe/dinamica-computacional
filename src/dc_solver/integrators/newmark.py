...
def newmark_beta_newton(
    ...
    try:
        ...
        fd0 = fd
        T_hist[0] = 0.5 * float(np.sum(M_diag[fd0] * (v_n[fd0] ** 2)))
        p_prev = p0[fd0].copy()
        R_prev = R0.copy()
        v_prev = v_n[fd0].copy()
    except Exception:
    ...
    for n in range(n_steps - 1):
        ...
        try:
            ...
            # Rint_final is internal force at converged state (free dofs)
            R_curr = Rint_final
            if p_prev is None:
                p_prev = (p_const + load_hist[n] - M_diag * r * ag[n])[fd]
            if R_prev is None:
                # compute from committed state
            ...
            # plastic work proxy from hinge infos (always positive)
            dWpl = 0.0
