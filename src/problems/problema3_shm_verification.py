"""Problema 3: Harness de verificación SHM."""
...
def main() -> None:
    ...
    theta_hist = _history_from_cycles([0.002, 0.004, 0.006, 0.008], steps_per_half=50)

    hinge = SHMBeamHinge1D(K0_0=2.5e8, My_0=3.0e5)
    ...
    K0_hist = np.zeros(steps)
    My_hist = np.zeros(steps)
    ...
    for i in range(steps):
        ...
        K0 = hinge.K0_0 * math.exp(-hinge.cK * hinge.a_comm)
        My = hinge.My_0 * math.exp(-hinge.cMy * hinge.a_comm)
        M_new, k_tan, th_p_new, a_new, M_comm = hinge.eval_increment(dth)

        hinge.th_p_comm = th_p_new
        hinge.a_comm = a_new
        hinge.M_comm = M_comm
    ...
    with csv_path.open("w", newline="", encoding="utf-8") as fh:
        ...
        writer.writerow(["step", "theta", "M", "a", "K0", "My", "th_p"])
        for i in range(steps):
            writer.writerow([i, theta_hist[i], M[i], a_hist[i], K0_hist[i], My_hist[i], th_p[i]])
    ...
    fig, ax = plt.subplots(figsize=(6, 5))
    ax.plot(a_hist, K0_hist, label="K0(a)")
    ax.plot(a_hist, My_hist, label="My(a)")
    ax.set_xlabel("a")
    ax.set_ylabel("Degradation")
    ax.grid(True, alpha=0.3)
    ax.legend()
    fig.tight_layout()
    fig.savefig(out / "problem3_shm_degradation.png", dpi=160)
    plt.close(fig)
