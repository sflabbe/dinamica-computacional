"""Problema 6: pórtico ELÁSTICO (sin rótulas) para test de integradores.

Objetivo
--------
- Regression / sanity check de Newmark / HHT / Explicit en el mismo pórtico de Problema 4,
  pero sin hinges.
- Incluye Step 1 gravity (estático) para comparación con el preload de Problema 4.

Extras (patch)
-------------
- Export de anregung (a_g) + balance de energía (CSV+PNG) en el run dinámico.
- Fix del script (antes tenía mezcla de funciones / código pegado al final).
- Outdir consistente con el resto (root/outputs/...).
"""
...
def make_time(t_end: float, dt: float) -> np.ndarray:
    return np.arange(0.0, float(t_end) + 1e-12, float(dt))


def ag_fun(t: np.ndarray, A: float) -> np.ndarray:
    """Mis misma 'anregung' que Problema 4."""
    t = np.asarray(t, dtype=float)
    return float(A) * np.cos(0.2 * np.pi * t) * np.sin(4.0 * np.pi * t)


def _story_stiffness_linear(model: Model, top_nodes: Tuple[int, int]) -> float:
    """Compute story stiffness with a unit horizontal force at roof nodes."""
    nd = model.ndof()
    fd = model.free_dofs()
    ...
    u_free = np.linalg.solve(K + 1e-14 * np.eye(fd.size), f[fd])
    u = np.zeros(nd)
    u[fd] = u_free
...
def _distribute_mass(
    *,
    ...
    """Lump a target total mass along all members (diag mass)."""
...
def build_portal_elastic(
    *,
    ...
    """Build elastic portal frame model (no hinges)."""
    ...
    def add_member(node_ids: List[int], A: float, I: float) -> None:
        for a, b in zip(node_ids[:-1], node_ids[1:]):
            beams.append(FrameElementLinear2D(a, b, E=float(E), A=float(A), I=float(I), nodes=nodes))
    ...
    # Gravity load distribution across all elements by A*L (consistent with Problem 4 style)
    vol_total = 0.0
    for e in beams:
        vol_total += float(e.A) * float(e._geom()[0])

    w_gravity_col = 0.0
    w_gravity_beam = 0.0

    if vol_total > 0.0 and P_gravity_total != 0.0:
        for e in beams:
            w_e = float(P_gravity_total) * float(e.A) / float(vol_total)  # N/m
            f_g = e.equiv_nodal_load_global((0.0, -w_e))
            dofs = e.dofs()
            for a, ia in enumerate(dofs):
                p0[ia] += f_g[a]

        w_gravity_col = float(P_gravity_total) * float(A_col) / float(vol_total)
        w_gravity_beam = float(P_gravity_total) * float(A_beam) / float(vol_total)
    ...
    # Target total mass from (T0, K_story)
    K_story = _story_stiffness_linear(model, top_nodes=(2, 3))
    omega0 = 2.0 * math.pi / float(T0)
    M_total = float(K_story) / float(omega0**2)
    ...
    # Optional explicit stability helper
    if explicit_mass_dt is not None:
        ...
        m_fd = mass[fd]
        mass[fd] = np.maximum(m_fd, np.maximum(m_req, 1e-12))
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Problema 6: pórtico ELÁSTICO (sin rótulas) para test integradores (Step1 gravity / Step2 dynamic)."
    )

    parser.add_argument("--state", default="dynamic", choices=["dynamic", "gravity"], help="Run mode.")
    parser.add_argument("--gravity", action="store_true", help="Alias for --state gravity.")

    parser.add_argument("--integrator", default="newmark", choices=["hht", "newmark", "explicit"], help="Time integrator.")
    ...
    parser.add_argument("--mass-mode", default="distributed", choices=["roof", "distributed"], help="Mass lumping strategy.")
    parser.add_argument("--no-rot-inertia", action="store_true", help="Disable rotational inertia lumping.")

    # Structural params (keep defaults compatible-ish with Problem 4)
    parser.add_argument("--H", type=float, default=3.0, help="Height [m].")
    parser.add_argument("--L", type=float, default=5.0, help="Bay width [m].")
    parser.add_argument("--E", type=float, default=30e9, help="Young's modulus [Pa].")
    parser.add_argument("--A-col", type=float, default=0.24, help="Column area [m^2].")
    parser.add_argument("--I-col", type=float, default=0.04, help="Column inertia [m^4].")
    parser.add_argument("--A-beam", type=float, default=0.125, help="Beam area [m^2].")
    parser.add_argument("--I-beam", type=float, default=0.026, help="Beam inertia [m^4].")

    parser.add_argument("--T0", type=float, default=0.5, help="Target period [s].")
    parser.add_argument("--zeta", type=float, default=0.02, help="Damping ratio.")
    parser.add_argument("--P-gravity-total", type=float, default=1500e3, help="Total vertical gravity load [N].")
    ...
    parser.add_argument("--alpha", type=float, default=-0.05, help="HHT-alpha parameter (ignored by Newmark/Explicit).")
    parser.add_argument("--drift-limit", type=float, default=0.10, help="Stop when max drift exceeds this limit.")
    parser.add_argument("--snapshot-limit", type=float, default=0.04, help="Record snapshot at this drift limit.")
    ...
    outdir = _outputs_dir(f"problem6_{state}_{integrator}_{args.mass_mode}")

    model, meta = build_portal_elastic(
        H=float(args.H),
        L=float(args.L),
        E=float(args.E),
        A_col=float(args.A_col),
        I_col=float(args.I_col),
        A_beam=float(args.A_beam),
        I_beam=float(args.I_beam),
        T0=float(args.T0),
        zeta=float(args.zeta),
        P_gravity_total=float(args.P_gravity_total),
        nseg=int(args.nseg),
        ...
        try:
            ...
            if isinstance(u_g, np.ndarray):
                ...
                plot_structure_states(
                    ...
                    snapshot_limit=float(args.snapshot_limit),
                    outfile=str(outdir / "step1_gravity_states_members.png"),
                    field="both",
                )
                plot_structure_states(
                    model,
                    ...
                    drift_height=float(meta.get("H", 3.0)),
                    snapshot_limit=float(args.snapshot_limit),
                    outfile=str(outdir / "step1_gravity_states_U.png"),
                    field="u",
                )
                plot_structure_states(
                    model,
                    ...
                    drift_height=float(meta.get("H", 3.0)),
                    snapshot_limit=float(args.snapshot_limit),
                    outfile=str(outdir / "step1_gravity_states_S.png"),
                    field="s",
                )
        except Exception as e:
            (outdir / "step1_gravity_plot_error.txt").write_text(str(e), encoding="utf-8")

        info = build_run_info(job="problem6_portico_elastico", output_dir=str(outdir), meta={"state": "gravity", **meta})
    ...
    # basic plots
    write_basic_plots(outdir, out, drift_limit=float(args.drift_limit))

    # Structure plots for history
    try:
        plot_structure_states(
            model,
            out,
            drift_height=float(meta.get("H", 3.0)),
            snapshot_limit=float(args.snapshot_limit),
            outfile=str(outdir / "problem6_states_members.png"),
            field="both",
        )
        plot_structure_states(
            model,
            ...
            drift_height=float(meta.get("H", 3.0)),
            snapshot_limit=float(args.snapshot_limit),
            outfile=str(outdir / "problem6_states_U.png"),
            field="u",
        )
        plot_structure_states(
            model,
            ...
            drift_height=float(meta.get("H", 3.0)),
            snapshot_limit=float(args.snapshot_limit),
            outfile=str(outdir / "problem6_states_S.png"),
            field="s",
        )
    except Exception as e:
        (outdir / "problem6_plot_error.txt").write_text(str(e), encoding="utf-8")

    # Diagnostics: anregung + energy balance
    try:
        prefix = f"problem6_Ag_{float(out.get('A_input_g', float(args.A_g))):.2f}g"
        export_anregung(outdir, prefix, out["t"], out["ag"])
        if isinstance(out.get("energy", None), dict):
            export_energy_balance(outdir, prefix, out["t"], out["energy"])
        e = out.get("energy", {}) if isinstance(out.get("energy", None), dict) else {}
        np.savez_compressed(
            outdir / f"{prefix}_history.npz",
            t=out.get("t", None),
            ag=out.get("ag", None),
            drift=out.get("drift", None),
            Vb=out.get("Vb", None),
            T=e.get("T", None),
    ...
    info = build_run_info(job="problem6_portico_elastico", output_dir=str(outdir), meta=ri_meta)
    write_run_info(outdir, base_name="problem6_runinfo", info=info)
